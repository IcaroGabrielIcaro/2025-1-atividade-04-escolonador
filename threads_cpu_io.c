...
void* cpu_thread(void* arg) {
    ...
    long n = 100000000; // número de termos para precisão
    double sum = 0.0;
    for (long k = 0; k < n; k++) {
...
int main() {
    ...
    if (pid == 0) { // Processo filho
        ...
        pthread_create(&thread_cpu3, NULL, cpu_thread, (void*)3); // nova thread

        // Cria threads I/O-bound
        pthread_create(&thread_io1, NULL, io_thread, (void*)1);
        pthread_create(&thread_io2, NULL, io_thread, (void*)2);
        pthread_create(&thread_io3, NULL, io_thread, (void*)3); // nova thread
